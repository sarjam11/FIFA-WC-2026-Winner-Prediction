...
DEFAULT_OUTPUT = "whoscored_player_stats.csv"
...
def fetch_player_stats(
    ...
    """
    Navigate to the statistics JSON URL for one player, extract the raw
    JSON text from the page body, parse it, flatten it, and return rows.
    """
    ...
    for attempt in range(1, retries + 2):
        try:
            ...
            # When a browser navigates to a JSON endpoint, the response is rendered as plain text inside the <body>, often wrapped in a <pre> tag.  We grab it with inner_text
            raw_text = ""
            ...
            # Quick sanity checks 
            # If Cloudflare served an HTML error page instead of JSON, the text will start with <!DOCTYPE or contain "Forbidden".
...
def run(
    ...
    with sync_playwright() as pw:
        ...
        # Warm-up: visit the homepage to establish cookies 
        # This is CRITICAL.  WhoScored's Cloudflare protection sets
        # cookies on the first visit.  If we jump straight to the JSON
        # endpoint, we'll get a 403 or a challenge page every time.
        log.info("=" * 60)
